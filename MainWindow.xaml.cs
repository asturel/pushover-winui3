...
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        App.ConfigChanged += () =>
        {
            // Azonnal frissítjük a listát, hogy a Converter újra lefusson az új formátummal
            ApplyFiltering();
        };
        ...
        AddLogLine($"[UI] Target config layout: {_configFilePath}");
        ...
        // Timer indítása, hogy percenként frissüljenek a "X perce kapott" feliratok
        ...
        _timeRefreshTimer.Tick += (s, e) =>
        {
            ...
            {
                RefreshVisibleTimestamps(); // Újraszámolja az időket, a meglévő hekked miatt nem ugrik el a görgetés!
            }
            RemoveExpiredMessages();
        };
        _timeRefreshTimer.Start();

        // Feliratkozunk a beállítás ablak mentés eseményére
        App.ConfigChanged += () =>
        {
            ApplyFiltering(); // Ha mentettünk, azonnal átváltja a formátumot
            RefreshVisibleTimestamps(); // Újraszámolja az időket, a meglévő hekked miatt nem ugrik el a görgetés!
        };
    }
